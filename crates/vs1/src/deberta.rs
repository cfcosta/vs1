//! DeBERTa-v2/v3 encoder, adapted from candle-transformers 0.11.0
//! (`models/debertav2.rs`, MIT OR Apache-2.0) for reduced precision.
//!
//! Changes from upstream: the attention scores, position bias and masked
//! softmax are computed in F32 whatever the weight dtype, and the
//! token-classification heads vs1 does not use are removed.
use candle_core::{Context, D, DType, Device, Module, Result, Tensor, bail};
use candle_nn::{
    Conv1d,
    Conv1dConfig,
    Embedding,
...
impl DebertaV2Embeddings {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let embedding_size =
            config.embedding_size.unwrap_or(config.hidden_size);
        ...
        let token_type_embeddings: Option<Embedding> =
        ...
        let embed_proj: Option<candle_nn::Linear> =
        ...
        let position_ids =
            Tensor::arange(0, config.max_position_embeddings as u32, &device)?
                .unsqueeze(0)?;
    ...
    pub fn forward(
        ...
        // Absolute positions exist only when they bias the input. DeBERTa-v3
        // uses relative positions alone, so its inputs may exceed the
        // absolute table (`max_position_embeddings`), as in PyTorch.
        if self.config.position_biased_input {
            let position_ids = match position_ids {
                Some(v) => v.clone(),
                None => self.position_ids.narrow(1, 0, seq_length)?,
            };
            if let Some(emb) = &self.position_embeddings {
                embeddings = embeddings.add(&emb.forward(&position_ids)?)?;
            }
        }
        ...
        if self.embedding_size != self.config.hidden_size {
            embeddings = if let Some(embed_proj) = &self.embed_proj {
                ...
                bail!(
                    "embed_proj must exist if embedding_size != config.hidden_size"
                );
            }
        }
...
impl XSoftmax {
    /// Masked softmax in F32, returned in `dtype`. Reduced-precision scores
    /// are upcast so the mask fill and normalization stay exact.
    pub fn apply(
        input: &Tensor,
        mask: &Tensor,
        dim: D,
        ...
        let min_value_tensor =
            Tensor::new(&[f32::MIN], device)?.broadcast_as(input.shape())?;
        let output = rmask.where_cond(&min_value_tensor, &input)?;
        let output = candle_nn::ops::softmax_last_dim(&output)?;
        debug_assert!(matches!(dim, D::Minus1));

        let t_zeroes =
            Tensor::new(&[0f32], device)?.broadcast_as(input.shape())?;
        rmask.where_cond(&t_zeroes, &output)?.to_dtype(dtype)
    }
}
...
impl DebertaV2DisentangledSelfAttention {
        ...
        if relative_attention {
            if max_relative_positions < 1 {
                max_relative_positions =
                    config.max_position_embeddings as isize;
            }
    ...
    pub fn forward(
        ...
        let query_layer =
            self.transpose_for_scores(&self.query_proj.forward(query_states)?)?;
        let key_layer =
            self.transpose_for_scores(&self.key_proj.forward(query_states)?)?;
        let value_layer =
            self.transpose_for_scores(&self.value_proj.forward(query_states)?)?;
        ...
        let scale = {
            ...
            Tensor::new(&[(q_size * scale_factor) as f32], &self.device)?
                .sqrt()?
        };
        ...
        // Sum the content and position scores in F32 before the softmax.
        ...
        let mut attention_probs = XSoftmax::apply(
            &attention_scores,
            attention_mask,
            D::Minus1,
            value_layer.dtype(),
    ...
    fn disentangled_attention_bias(
        ...
        // Accumulated in F32; the attention softmax upcasts anyway.
        let mut score = Tensor::new(&[0 as f32], &self.device)?;

        if self.config.pos_att_type.iter().any(|s| s == "c2p") {
            let pos_key_layer =
                pos_key_layer.context("c2p without pos_key_layer")?;
            ...
            score = score.broadcast_add(
                &c2p_att.to_dtype(DType::F32)?.broadcast_div(&scale)?,
            )?;
        }

        if self.config.pos_att_type.iter().any(|s| s == "p2c") {
            let pos_query_layer =
                pos_query_layer.context("p2c without pos_key_layer")?;
            ...
            score = score.broadcast_add(
                &p2c_att.to_dtype(DType::F32)?.broadcast_div(&scale)?,
            )?;
        }
...
impl DebertaV2Attention {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dsa = DebertaV2DisentangledSelfAttention::load(
            vb.pp("attention.self"),
            config,
        )?;
        let output =
            DebertaV2SelfOutput::load(vb.pp("attention.output"), config)?;
        Ok(Self { dsa, output })
    }
...
impl ConvLayer {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let conv_act: String =
            config.conv_act.clone().unwrap_or("tanh".to_string());
...
impl DebertaV2Encoder {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let conv: Option<ConvLayer> =
            if config.conv_kernel_size.unwrap_or(0) > 0 {
    ...
    pub fn forward(
        ...
        let relative_pos =
            self.get_rel_pos(hidden_states, query_states, relative_pos)?;
        ...
        for (i, layer_module) in self.layer.iter().enumerate() {
            ...
            {
                output_states =
                    conv.forward(hidden_states, &output_states, &input_mask)?;
            }
    ...
    fn get_rel_embedding(&self) -> Result<Option<Tensor>> {
        ...
        let rel_embeddings = self
            ...
            .as_ref()
            .context(
                "self.rel_embeddings not present when using relative_attention",
            )?
            .embeddings()
            .clone();
...
impl DebertaV2Model {
    ...
    pub fn forward(
        ...
        let encoder_output = self.encoder.forward(
            &embedding_output,
            &attention_mask,
            None,
            None,
...
pub(crate) fn build_relative_position(
    ...
    let k_ids: Tensor =
        Tensor::arange(0, key_size as i64, device)?.unsqueeze(D::Minus1)?;
...
pub(crate) fn make_log_bucket_position(
    ...
    let log_pos = {
        ...
        let to_ceil = first_div_second.broadcast_mul(
            Tensor::from_slice(&[(mid - 1) as f32], (1,), device)?.as_ref(),
        )?;
    ...
    Ok({
        let abs_pos_lte_mid =
            abs_pos.to_dtype(DType::F32)?.broadcast_le(&mid_as_tensor)?;
        let relative_pos = relative_pos.to_dtype(relative_pos.dtype())?;
        let log_pos_mul_sign =
            log_pos.broadcast_mul(&sign.to_dtype(DType::F32)?)?;
        abs_pos_lte_mid.where_cond(
            &relative_pos.to_dtype(DType::F32)?,
            &log_pos_mul_sign,
        )?
    })
}
